...
framework_paths=$(py_jsonvalue -p $1 -k framework_paths/Build)
...
fw_build_cmd="ios_universal_framework.sh"

for path in "${fw_paths[@]}"
do
    echo "=> building framework at $path - $frameworks_dir"
    sh ${fw_build_cmd} ${path} ${frameworks_dir} &
done
wait
...
do
    ...
    production_path=$(py_jsonvalue -p $1 -k framework_paths/Production)
    development_path=$(py_jsonvalue -p $1 -k framework_paths/Development)
